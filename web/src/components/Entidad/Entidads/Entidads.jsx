...
const EntidadsList = ({ entidads = [] }) => {
  ...
  const filteredEntidades = useMemo(() => {
    ...
    return entidads.filter((entidad) => entidad.estado === 'ACTIVO')
    ...
    renderRowActions: ({ row }) => (
      <Box sx={{ display: 'flex', gap: '8px' }}>
        <Tooltip title="Ver detalles">
          <IconButton
            component={Link}
            to={routes.entidad({ id: row.original.id })}
          >
        ...
        <Tooltip title="Editar">
          <IconButton
            component={Link}
            to={routes.editEntidad({ id: row.original.id })}
          >
  ...
  return (
    <Box sx={{ p: 1 }}>
      ...
      >
        ...
        <DialogContent>
          <Typography>
            ¿Estás seguro de desactivar la entidad {deleteState.id}? Esta acción
            no eliminará la entidad de la base de datos, solo cambiará su estado
            a inactivo.
          </Typography>
        </DialogContent>
        <DialogActions>
          ...
          <Button
            onClick={() => desactivarEntidad(deleteState.id)}
            color="error"
            variant="contained"
            sx={{
