...
import {
  ...
  Edit as EditIcon,
  Business as EntidadIcon, // Icono genérico para Entidad
  GridOn as ExcelIcon,
...
const Entidades = ({ entidads, usuarios }) => {
  ...
  const handleSoftDelete = (rows) => {
    rows.forEach((entidad) => {
      ...
      // Asumimos ID 1 para modificación automática si no hay auth context disponible aquí
      updateEntidad({
        variables: { id: entidad.id, input: { estado: newState, usuario_modificacion: 1 } },
      })
    })
