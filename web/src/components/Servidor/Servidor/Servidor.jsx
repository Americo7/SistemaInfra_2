...
// ✔ NUEVA CONSULTA: Trae los parámetros necesarios (Inventario y Tipo Servidor)
...
const Servidor = ({ servidor }) => {
  ...
  // ✔ USAMOS useQuery para los diccionarios (Usuarios y Parámetros)
  const { data: usuariosData } = useQuery(GET_USUARIOS_QUERY)
  const { data: parametrosData } = useQuery(GET_PARAMETROS_QUERY)

  // ❌ ELIMINADA la consulta redundante GET_SERVIDOR_PADRE_QUERY

  const usuariosMap = useMemo(() => {
    return (
      usuariosData?.usuarios?.reduce((map, usuario) => {
        map[usuario.id] = `${usuario.nombres} ${usuario.primer_apellido}`.trim()
        return map
      }, {}) || {}
    )
  }, [usuariosData])
  ...
  const parametrosMap = useMemo(() => {
    return (
      parametrosData?.parametros?.reduce((map, param) => {
  ...
  const getNombreParametro = (codigo) => {
    return parametrosMap[codigo] || codigo || '-'
  }
  ...
  // ✔ RELACIONES CORRECTAS: Usamos los nombres de relación de Prisma
          ...
          subheader={
            <Box sx={{ display: 'flex', alignItems: 'center', mt: 1 }}>
              <Typography variant="subtitle1" color="text.secondary">
                {getNombreParametro(servidor.cod_tipo_servidor)} • {getNombreParametro(servidor.cod_inventario_agetic)} {/* ✔ MAPEO DE PARÁMETROS */}
              </Typography>
              <Chip
                ...
                <Grid item xs={6}>
                  <Paper variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
                    ...
                    <Typography variant="h6" sx={{ fontWeight: 600 }}>
                      {servidor.ram ? `${servidor.ram} GB` : 'N/A'}
                    </Typography>
                  </Paper>
                </Grid>
                <Grid item xs={6}>
                  <Paper variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
                    ...
                    <Typography variant="h6" sx={{ fontWeight: 600 }}>
                      {servidor.almacenamiento ? `${servidor.almacenamiento} GB` : 'N/A'}
                    </Typography>
                  </Paper>
                </Grid>
              </Grid>
              ...
              <TableContainer component={Paper} variant="outlined" sx={{ borderRadius: 2 }}>
                <Table size="small">
                  <TableBody>
                    ...
                    <TableRow>
                      <TableCell sx={{ fontWeight: 600 }}>IP Primaria</TableCell>
                      <TableCell>{servidor.ip_primaria || '-'}</TableCell> {/* ✔ CAMPO NUEVO */}
                    </TableRow>
                    <TableRow>
                      <TableCell sx={{ fontWeight: 600 }}>Sistema Operativo</TableCell>
                      <TableCell>{servidor.sistema_operativo || '-'}</TableCell> {/* ✔ CAMPO NUEVO */}
                    </TableRow>
                    <TableRow>
                      ...
                      <TableCell>
                        <Stack direction="row" alignItems="center" spacing={1}>
                          ...
                          <span>{servidor.data_centers?.nombre || 'No asignado'}</span> {/* ✔ ACCESO CORRECTO */}
                        </Stack>
                      </TableCell>
                    </TableRow>
                    <TableRow>
                      ...
                      <TableCell>
                        {servidor.servidores_padre ? ( /* ✔ ACCESO DIRECTO AL OBJETO */
                          <Stack direction="row" alignItems="center" spacing={1}>
                            <ServerIcon fontSize="small" color="action" />
                            <span>{servidor.servidores_padre.nombre}</span>
                          </Stack>
                        ) : 'Ninguno'}
                      </TableCell>
                    </TableRow>
            ...
            <Grid item xs={12} md={6}>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={6}>
                    ...
                    <Typography variant="body1" sx={{ fontWeight: 500 }}>
                      {usuariosMap[servidor.usuario_creacion] || 'N/A'} {/* ✔ MAPEO DE USUARIO */}
                    </Typography>
                  </Grid>
                  <Grid item xs={6}>
                    <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1 }}>
                  ...
                  <Grid item xs={6}>
                    ...
                    <Typography variant="body1" sx={{ fontWeight: 500 }}>
                      {usuariosMap[servidor.usuario_modificacion] || 'N/A'} {/* ✔ MAPEO DE USUARIO */}
                    </Typography>
                  </Grid>
                  <Grid item xs={6}>
                    <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1 }}>
              ...
              <Stack spacing={1} divider={<Divider orientation="horizontal" flexItem />}>
                <Typography variant="body2">
                    <span style={{ fontWeight: 600 }}>Total VMs Hosteadas:</span> {machines.length}
                </Typography>
                <Typography variant="body2">
                    <span style={{ fontWeight: 600 }}>Total Despliegues:</span> {deployments.length}
                </Typography>
                <Typography variant="body2">
                    <span style={{ fontWeight: 600 }}>Roles en Clusters:</span>
                    {clusterNodos.map(n => n.rol || 'N/A').join(', ')}
                </Typography>
              </Stack>
            </Grid>
          </Grid>
      ...
      <Card sx={{ borderRadius: 3, boxShadow: theme.shadows[3] }}>
        ...
        <CardContent>
                  ...
                  <Table>
                    <TableHead>
                      <TableRow sx={{ backgroundColor: theme.palette.grey[50] }}>
                        <TableCell sx={{ fontWeight: 600 }}>Nombre VM</TableCell>
                    ...
                    <TableBody>
                      {machines.map((vm) => ( /* ✔ Iteramos sobre 'maquinas' */
                          ...
                          <TableCell>
                            {vm.estado ? (
                              <Chip
                                ...
                                size="small"
                                sx={{
                                  backgroundColor: getEstadoColor1(vm.estado),
                                  color: 'white',
                                  fontSize: '0.7rem',
                                }}
                              />
          ...
          {activeTab === 1 && (
            <>
              <Typography variant="subtitle1" sx={{ mb: 2, fontWeight: 600 }}>
                Roles en Clusters (Proxmox/Kubernetes)
              </Typography>
              {clusterNodos.length > 0 ? (
                <TableContainer component={Paper} variant="outlined" sx={{ borderRadius: 2 }}>
                  <Table>
                    <TableHead>
                      <TableRow sx={{ backgroundColor: theme.palette.grey[50] }}>
                        ...
                        <TableCell sx={{ fontWeight: 600 }}>Rol (K8s/PVE)</TableCell>
                        <TableCell sx={{ fontWeight: 600 }}>Estado Nodo</TableCell>
                      </TableRow>
                    </TableHead>
                    <TableBody>
                      {clusterNodos.map((nodo) => ( /* ✔ Iteramos sobre 'cluster_nodos' */
                        <TableRow key={nodo.id} hover>
                          <TableCell>
                            <Typography variant="body2" sx={{ fontWeight: 500 }}>
                              {nodo.cluster?.nombre || 'N/A'}
                            </Typography>
                          </TableCell>
                          <TableCell>
                            <Chip
                              label={nodo.cluster?.cod_tipo_cluster || 'N/A'}
                              size="small"
                              sx={{
                          ...
                          <TableCell>
                            {nodo.estado ? (
                              <Chip
                                ...
                                size="small"
                                sx={{
                                  backgroundColor: getEstadoColor1(nodo.estado),
                                  color: 'white',
                                  fontSize: '0.7rem',
                                }}
                              />
          ...
          {activeTab === 3 && ( /* ✔ Nuevo Tab para Despliegues */
            <>
              <Typography variant="subtitle1" sx={{ mb: 2, fontWeight: 600 }}>
                Historial de Despliegues de Aplicaciones
              </Typography>
              {deployments.length > 0 ? (
                    ...
                    <TableBody>
                      {deployments.map((dep) => (
                        <TableRow key={dep.id} hover>
                          <TableCell>{dep.componentes?.nombre || 'N/A'}</TableCell>
                          <TableCell>{formatDate(dep.fecha_despliegue)}</TableCell>
                          <TableCell>{dep.solicitante || 'N/A'}</TableCell>
                          <TableCell>
                            <Chip
                                label={dep.estado_despliegue || 'N/A'}
                                size="small"
                                color={dep.estado_despliegue === 'EXITOSO' ? 'success' : 'warning'}
                            />
                          </TableCell>
                        </TableRow>
                      ))}
                ...
                <Paper variant="outlined" sx={{ p: 4, textAlign: 'center', borderRadius: 2 }}>
                  <UpdateIcon sx={{ fontSize: 40, color: theme.palette.grey[400], mb: 1 }} />
                  <Typography variant="body1" color="text.secondary">
                    No hay despliegues registrados para este servidor físico.
                  </Typography>
                </Paper>
              )}
            </>
