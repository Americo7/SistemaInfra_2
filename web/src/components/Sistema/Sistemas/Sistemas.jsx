...
// Helper simple: recibe el objeto usuario (ej: row.creadoPor) y devuelve string
const formatUser = (userObj) => {
  if (!userObj) return '-' // Si es null o undefined
...
const Sistemas = ({ sistemas }) => {
  ...
  // Helpers para exportador (reutilizando el formateador)
  const exportHelpers = {
    getUsuarioNombre: (val, row) => {
        // Si la columna es 'creadoPor', val será el objeto creadoPor
  ...
  const handleSoftDelete = (rows) => {
    rows.forEach((sistema) => {
      ...
      updateSistema({
        variables: { id: sistema.id, input: { estado: newState } },
      })
    })
  ...
  const filteredData = useMemo(() => {
    ...
    return sistemas.filter((s) =>
      showDeleted ? s.estado === 'INACTIVO' : s.estado !== 'INACTIVO'
    )
  ...
  const columns = useMemo(() => [
    ...
    { 
        id: 'creadoPor',
        accessorKey: 'creadoPor', // Usamos el objeto directamente
        header: 'Creado por', 
        size: 160, 
    ...
    { 
        id: 'modificadoPor',
        accessorKey: 'modificadoPor', // Usamos el objeto directamente
        header: 'Modif. por', 
        size: 160, 
  ...
  const namesToDelete = rowsToDelete.length === 1 
    ? rowsToDelete[0]?.nombre || rowsToDelete[0]?.sigla || `el sistema ID ${rowsToDelete[0]?.id}` 
