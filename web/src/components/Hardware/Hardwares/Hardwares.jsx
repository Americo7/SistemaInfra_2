...
const UPDATE_HARDWARE_MUTATION = gql`
  mutation UUpdateHardwareMutation_fromHardware(
    $id: Int!
...
const HardwaresList = ({ hardwares = [] }) => {
  if (!Array.isArray(hardwares)) {
    console.error('Error: hardwares no es un array', hardwares)
    return (
  ...
  const [updateHardware] = useMutation(UPDATE_HARDWARE_MUTATION, {
    onCompleted: () => {
      toast.success('Hardware desactivado correctamente')
  ...
  const filteredHardwares = useMemo(() => {
    ...
    return hardwares.filter((hardware) => hardware.estado === 'ACTIVO')
  }, [hardwares, showInactive])
  ...
  const exportToPDF = (rows, table) => {
    ...
    doc.text('Reporte de Hardware', 14, 15)
    ...
    doc.save(`hardwares-${new Date().toISOString()}.pdf`)
  }
  ...
  const exportToExcel = (rows, table) => {
    ...
    XLSX.utils.sheet_add_aoa(ws, [['Reporte de Hardware']], { origin: 'A1' })
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Hardwares')
    XLSX.writeFile(wb, `hardwares-${new Date().toISOString()}.xlsx`)
  }
  ...
  const exportToCSV = (rows, table) => {
    ...
    link.download = `hardwares-${new Date().toISOString()}.csv`
    link.click()
  }
  ...
  const columns = useMemo(
    () => [
      ...
      { accessorKey: 'cod_activo_agetic', header: 'Código AGETIC', size: 120 },
      { accessorKey: 'cod_tipo_hw', header: 'Tipo HW', size: 100 },
    ...
    renderRowActions: ({ row }) => (
      <Box sx={{ display: 'flex', gap: '8px' }}>
        <Tooltip title="Ver detalles">
          <IconButton
            component={Link}
            to={routes.hardware({ id: row.original.id })}
          >
        ...
        <Tooltip title="Editar">
          <IconButton
            component={Link}
            to={routes.editHardware({ id: row.original.id })}
          >
  ...
  return (
    <Box sx={{ p: 1 }}>
      ...
      >
        ...
        <DialogContent>
          <Typography>
            ¿Estás seguro de desactivar el hardware {deleteState.id}? Esta
        ...
        <DialogActions>
          ...
          <Button
            onClick={() => desactivarHardware(deleteState.id)}
            color="error"
            variant="contained"
            sx={{
