...
import { generatePDF as generateDetailedPDF } from 'src/lib/exporter/maquinaDetalleExporter'
...
const Maquinas = ({ maquinas }) => {
  ...
  // --- HANDLER PARA REPORTE DETALLADO (OPTIMIZADO - UN SOLO ARCHIVO) ---
  const handleDetailedReport = async (rows) => {
    if (rows.length === 0) {
      toast.error('Debe seleccionar al menos una máquina')
      return
    }

    const toastId = toast.loading(`Generando reporte consolidado de ${rows.length} máquina(s)...`)

    try {
      // 1. Extraemos los datos completos
      const maquinasCompletas = rows.map(r => r.original)

      // 2. Generamos UN SOLO PDF con todas las máquinas
      const pdfDataUri = await generateDetailedPDF(maquinasCompletas)

      // 3. Descargamos el archivo único
      const link = document.createElement('a')
      link.href = pdfDataUri

      // Nombre del archivo dinámico
      const fileName = maquinasCompletas.length === 1
        ? `Maquina_${maquinasCompletas[0].nombre}_Detalle.pdf`
        : `Reporte_Consolidado_${maquinasCompletas.length}_Maquinas_${new Date().getTime()}.pdf`
      ...
      toast.error('Error al generar el reporte: ' + error.message, { id: toastId })
    }
  }
  ...
  const listActionsConfig = useMemo(() => {
    ...
    const ExportMenu = (
      <Menu anchorEl={exportMenuAnchorEl} open={Boolean(exportMenuAnchorEl)} onClose={closeAllDialogs}>
        ...
        <MenuItem
          onClick={() => handleDetailedReport(table.getSelectedRowModel().rows)}
