...
const UPDATE_CLUSTER_MUTATION = gql`
  mutation UpdateClusterMutation_fromCluster(
    $id: Int!
...
const ClustersList = ({ clusters = [] }) => {
  ...
  const [updateCluster] = useMutation(UPDATE_CLUSTER_MUTATION, {
    onCompleted: () => {
      toast.success('Cluster desactivado correctamente')
  ...
  const filteredClusters = useMemo(() => {
    ...
    return clusters.filter((cluster) => cluster.estado === 'ACTIVO')
  ...
  const columns = useMemo(
    () => [
      ...
      { accessorKey: 'cod_tipo_cluster', header: 'Tipo Cluster', size: 120 },
    ...
    renderRowActions: ({ row }) => (
      <Box sx={{ display: 'flex', gap: '8px' }}>
        ...
        <Tooltip title="Editar">
          <IconButton
            component={Link}
            to={routes.editCluster({ id: row.original.id })}
          >
  ...
  return (
    <Box sx={{ p: 1 }}>
      ...
      >
        ...
        <DialogContent>
          <Typography>
            ¿Estás seguro de desactivar el cluster {deleteState.id}? Esta acción
            no eliminará el cluster de la base de datos, solo cambiará su estado
            a inactivo.
          </Typography>
        </DialogContent>
        <DialogActions>
          ...
          <Button
            onClick={() => desactivarCluster(deleteState.id)}
            color="error"
            variant="contained"
            sx={{
