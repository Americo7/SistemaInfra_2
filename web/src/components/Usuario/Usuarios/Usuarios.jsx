...
import { exportToExcel, exportToPDF, exportToCSV } from 'src/lib/exporter/usuariosExporter'
...
const Usuarios = ({ usuarios }) => {
  ...
  const handleSoftDelete = (rows) => {
    rows.forEach((usuario) => {
      ...
      // Nota: Si tienes contexto de autenticación, inyecta el ID del usuario actual aquí
      updateUsuario({
        variables: { id: usuario.id, input: { estado: newState } },
      })
    })
    
    toast.success(`${rows.length} usuarios ${showDeleted ? 'reactivados' : 'desactivados'}.`)
  ...
  const confirmHardDelete = () => {
    ...
    rowsToDelete.forEach((usuario) => {
      deleteUsuario({ variables: { id: usuario.id } })
    })
    
    // Muestra el toast de éxito UNA SOLA VEZ
    toast.success(`${rowsToDelete.length} usuario(s) eliminado(s) permanentemente.`) 
  ...
  const filteredData = useMemo(() => {
    ...
    return usuarios.filter((u) =>
      showDeleted ? u.estado === 'INACTIVO' : u.estado !== 'INACTIVO'
    )
  }, [usuarios, showDeleted])
  ...
  const columns = useMemo(() => [
    ...
    { 
        ...
        Cell: ({ row }) => (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <PersonIcon color={row.original.estado === 'INACTIVO' ? 'disabled' : 'primary'} fontSize="small" />
    ...
    { 
        ...
        size: 220,
        accessorFn: (row) => `${row.nombres || ''} ${row.primer_apellido || ''} ${row.segundo_apellido || ''}`.trim()
    },
    ...
    renderRowActions: ({ row }) => (
      <Stack direction="row" spacing={0.5}>
        <Tooltip title="Ver Detalles">
          <IconButton component={Link} to={routes.usuario({ id: row.original.id })} size="small">
            <VisibilityIcon fontSize="small" color="primary" />
          </IconButton>
        </Tooltip>
        <Tooltip title="Editar">
          <IconButton component={Link} to={routes.editUsuario({ id: row.original.id })} size="small">
  ...
  const namesToDelete = rowsToDelete.length === 1 
    ? rowsToDelete[0]?.nombre_usuario || rowsToDelete[0]?.email || `el usuario ID ${rowsToDelete[0]?.id}` 
