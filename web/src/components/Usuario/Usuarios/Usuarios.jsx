...
const UsuariosList = ({ usuarios = [] }) => {
  ...
  const exportToExcel = (rows, table) => {
    ...
    XLSX.utils.sheet_add_aoa(ws, [['Reporte de Usuarios']], { origin: 'A1' })
  ...
  return (
    <Box sx={{ p: 1 }}>
      ...
      >
        ...
        <DialogContent>
          <Typography>
            ¿Estás seguro de desactivar el usuario {deleteState.id}? Esta acción
            no eliminará el usuario de la base de datos, solo cambiará su estado
            a inactivo.
          </Typography>
        </DialogContent>
        <DialogActions>
          ...
          <Button
            onClick={() => desactivarUsuario(deleteState.id)}
            color="error"
            variant="contained"
            sx={{
