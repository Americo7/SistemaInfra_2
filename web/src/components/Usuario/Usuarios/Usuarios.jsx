...
const Usuarios = ({ usuarios, creators }) => {
  ...
  const handleSoftDelete = (rows) => {
    rows.forEach((usuario) => {
      ...
      // Asumimos ID 1 para usuario_modificacion si no hay contexto de auth
      updateUsuario({
        variables: { id: usuario.id, input: { estado: newState, usuario_modificacion: 1 } },
      })
    })
  ...
  const filteredData = useMemo(() => {
    ...
    return usuarios.filter((u) =>
      showDeleted ? u.estado === 'INACTIVO' : u.estado === 'ACTIVO'
    )
