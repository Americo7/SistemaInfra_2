...
const RolePage = ({ id }) => {
  ...
  const [updateRole] = useMutation(UPDATE_MUTATION, {
    onCompleted: () => {
      toast.success('Rol desactivado (INACTIVO)')
      navigate(routes.roles())
    },
  ...
  const dialogTitle = isHardDelete ? '¿Eliminar permanentemente?' : '¿Desactivar rol?'
  const dialogContent = isHardDelete
    ? `Estás a punto de ELIMINAR PERMANENTEMENTE el rol "${nombreRole}". Esta acción afectará a todos los usuarios que tengan este rol asignado y NO se puede deshacer.`
    : `¿Estás seguro de DESACTIVAR el rol "${nombreRole}"? Pasará a estado INACTIVO.`
