...
const ComponentePage = ({ id }) => {
  ...
  const [updateComponente] = useMutation(UPDATE_MUTATION, {
    onCompleted: () => {
      toast.success('Componente desactivado (INACTIVO)')
      navigate(routes.componentes())
    },
  ...
  const dialogTitle = isHardDelete ? '¿Eliminar permanentemente?' : '¿Desactivar componente?'
  const dialogContent = isHardDelete
    ? `Estás a punto de ELIMINAR PERMANENTEMENTE el componente "${nombreComponente}". Esta acción eliminará los despliegues asociados y NO se puede deshacer.`
    : `¿Estás seguro de DESACTIVAR el componente "${nombreComponente}"? Pasará a estado INACTIVO.`
