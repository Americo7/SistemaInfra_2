...
const EventoPage = ({ id }) => {
  ...
  const descripcionRaw = evento.descripcion || 'Cargando...'
  const tituloEvento = descripcionRaw.length > 50 
    ? `${descripcionRaw.substring(0, 50)}...` 
    : descripcionRaw
  
  // Lógica de estado unificada: 'ACTIVO' o 'INACTIVO'
  const isActivo = evento.estado === 'ACTIVO'
  ...
  const [updateEvento] = useMutation(UPDATE_MUTATION, {
    onCompleted: (data) => {
      const nuevoEstado = data.updateEvento.estado
      toast.success(`Evento marcado como ${nuevoEstado === 'ACTIVO' ? 'ACTIVO' : 'INACTIVO'}`)
      refetch()
    },
  ...
  const handleConfirmAction = () => {
    ...
    if (confirmDialog.type === 'soft') {
      updateEvento({ variables: { id, input: { estado: 'INACTIVO' } } })
    } else if (confirmDialog.type === 'restore') {
      updateEvento({ variables: { id, input: { estado: 'ACTIVO' } } })
    ...
    >
      <MenuItem onClick={handleToggleStateClick} disableRipple sx={{ py: 1.5 }}>
        ...
        <ListItemText
          ...
          secondary={isActivo ? "Marcar como INACTIVO" : "Habilitar/Reactivar evento"}
  ...
  if (isHardDelete) {
    ...
    dialogContent = `Estás a punto de ELIMINAR PERMANENTEMENTE el evento "${tituloEvento}". Esta acción NO se puede deshacer.`
    confirmButtonColor = 'error'
    confirmButtonText = 'Eliminar'
    DialogIcon = HardDeleteIcon
  } else if (isRestore) {
    dialogTitle = '¿Restaurar evento?'
    dialogContent = `¿Deseas RESTAURAR el evento "${tituloEvento}"? Pasará a estado ACTIVO.`
    confirmButtonColor = 'success'
    confirmButtonText = 'Restaurar'
    DialogIcon = RestoreIcon
  } else {
    // Soft Delete (Desactivar)
    dialogTitle = '¿Desactivar evento?'
    dialogContent = `¿Estás seguro de DESACTIVAR el evento "${tituloEvento}"? Pasará a estado INACTIVO.`
