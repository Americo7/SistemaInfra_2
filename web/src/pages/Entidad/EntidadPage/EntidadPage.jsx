...
const EntidadPage = ({ id }) => {
  ...
  const [updateEntidad] = useMutation(UPDATE_MUTATION, {
    onCompleted: () => {
      toast.success('Entidad desactivada (INACTIVO)')
      navigate(routes.entidads())
    },
  ...
  const dialogTitle = isHardDelete ? '¿Eliminar permanentemente?' : '¿Desactivar entidad?'
  const dialogContent = isHardDelete
    ? `Estás a punto de ELIMINAR PERMANENTEMENTE la entidad "${nombreEntidad}". Esta acción podría afectar a los sistemas asociados y NO se puede deshacer.`
    : `¿Estás seguro de DESACTIVAR la entidad "${nombreEntidad}"? Pasará a estado INACTIVO.`
