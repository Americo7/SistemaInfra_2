...
const getFormattedData = (rows, columns, helpers) => {
  return rows.map((row) => {
    return columns.map((col) => {
      ...
      if (col.id === 'id_evento') {
        return helpers.getEventoDescripcion ? helpers.getEventoDescripcion(val) : val
      }
      if (col.id === 'id_data_center') {
        return helpers.getDataCenterNombre ? helpers.getDataCenterNombre(val) : val
      }
      if (col.id === 'id_servidor') {
        return helpers.getServidorNombre ? helpers.getServidorNombre(val) : val
      }
      if (col.id === 'id_maquina') {
        return helpers.getMaquinaNombre ? helpers.getMaquinaNombre(val) : val
      }
...
export const exportToPDF = (rows, columns, helpers, suffix = '') => {
  ...
  doc.setFontSize(16)
  doc.text('Reporte de Infraestructura Afectada', 14, 15)
  doc.setFontSize(10)
  ...
  autoTable(doc, {
    ...
    columnStyles: {
        1: { cellWidth: 50 }, // Evento (descripción larga)
    }
  })

  doc.save(`Infra_Afectada_${dateStr.replace(/[\/\s:]/g, '_')}${suffix}.pdf`)
}
...
export const exportToExcel = (rows, columns, helpers, suffix = '') => {
  ...
  XLSX.utils.sheet_add_aoa(ws, [['Reporte de Infraestructura Afectada']], { origin: 'A1' })
  ...
  XLSX.utils.book_append_sheet(wb, ws, 'InfraAfectada')
  const dateStr = new Date().toLocaleString('es-BO').replace(/[\/\s:]/g, '_')
  XLSX.writeFile(wb, `Infra_Afectada_${dateStr}${suffix}.xlsx`)
}
...
export const exportToCSV = (rows, columns, helpers, suffix = '') => {
  ...
  link.setAttribute('download', `Infra_Afectada_${dateStr}${suffix}.csv`)
