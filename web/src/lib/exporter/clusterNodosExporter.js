  ...
  return {
    ...
    data: rows.map((row) =>
      visibleColumns.map((column) => {
        ...
        if (column.id === 'nodoTipo')
          return helpers.getNombreTipoNodo(cellValue)
        ...
        if (column.id === 'recurso')
          return helpers.getRecurso(row.original)
...
export const exportToPDF = (rows, visibleColumns, helpers, suffix = '') => {
  ...
  doc.text('Reporte de Nodos de Cluster', 14, 15)
  ...
  doc.save(`clusterNodos${suffix}-${new Date().toISOString().split('T')[0]}.pdf`)
}
...
export const exportToExcel = (rows, visibleColumns, helpers, suffix = '') => {
  ...
  XLSX.utils.sheet_add_aoa(ws, [['Reporte de Nodos de Cluster']], { origin: 'A1' })
  ...
  XLSX.utils.book_append_sheet(wb, ws, 'Cluster Nodos')
  XLSX.writeFile(wb, `clusterNodos${suffix}-${new Date().toISOString().split('T')[0]}.xlsx`)
}

export const exportToCSV = (rows, visibleColumns, helpers, suffix = '') => {
  const { headers, data } = getFormattedData(rows, visibleColumns, helpers)
  const csvContent = [
    'Reporte de Nodos de Cluster',
    `Generado: ${formatDateTime(new Date())}`,
    '',
    headers.join(','),
    ...data.map((row) =>
  ...
  link.download = `clusterNodos${suffix}-${new Date().toISOString().split('T')[0]}.csv`
  link.click()
}
