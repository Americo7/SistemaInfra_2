  ...
  return rows.map((row) => {
    return columns.map((col) => {
      ...
      if (['usuario_creacion', 'usuario_modificacion'].includes(col.id)) {
        return helpers.getUserName ? helpers.getUserName(val) : val
      }

      // 3. Count de Clusters
      if (col.id === 'clusters_count') {
        return row.original.clusters?.length || 0
      }
...
export const exportToPDF = (rows, columns, helpers, suffix = '') => {
  ...
  doc.setFontSize(16)
  doc.text('Reporte de Endpoints Kubernetes', 14, 15)
  doc.setFontSize(10)
  ...
  doc.save(`K8s_Endpoints_${dateStr.replace(/[\/\s:]/g, '_')}${suffix}.pdf`)
}
...
export const exportToExcel = (rows, columns, helpers, suffix = '') => {
  ...
  XLSX.utils.sheet_add_aoa(ws, [['Reporte de Endpoints Kubernetes']], { origin: 'A1' })
  ...
  XLSX.utils.book_append_sheet(wb, ws, 'K8sEndpoints')
  const dateStr = new Date().toLocaleString('es-BO').replace(/[\/\s:]/g, '_')
  XLSX.writeFile(wb, `K8s_Endpoints_${dateStr}${suffix}.xlsx`)
}
...
export const exportToCSV = (rows, columns, helpers, suffix = '') => {
  ...
  link.setAttribute('download', `K8s_Endpoints_${dateStr}${suffix}.csv`)
